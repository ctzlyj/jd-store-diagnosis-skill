...
- **部门类目构成因店而异，不要写死**：全球锦宏优选=「个人护理工具」单类目；
  惠民日用供应链经营部=「个人护理工具＋日用工具＋收纳架」三类目。
  先用商智按二级类目加总后做子集匹配，找出恰好等于 GE DealAmt 的类目组合，
  即可确定该店的部门构成，再按上式对账（第二例两期差额亦均 0.00，DealSpuNum 10/12 同样对上）。
...
2026-09 全球锦宏优选、惠民日用供应链经营部（均为 C 店，无京麦权限）两次实战标定并交付；ge-sz-fetch.py 两店均 122/122 ok，链路无需改动。后续平台改版以真实探测为准。
